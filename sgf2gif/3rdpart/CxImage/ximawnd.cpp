...
bool CxImage::CreateFromHANDLE(HANDLE hMem)
{
	...
	if (lpVoid){
		...
		if((pHead->biCompression != BI_RGB) || (pHead->biBitCount == 32)){ //<J锟絩gen Alfredsson>
			// BITFIELD case
...
int32_t CxImage::Draw(HDC hdc, int32_t x, int32_t y, int32_t cx, int32_t cy, RECT* pClipRect, bool bSmooth, bool bFlipY)
{
	...
	paintbox.top = MIN_MACRO(clipbox.bottom,MAX_MACRO(clipbox.top,y));
	paintbox.left = MIN_MACRO(clipbox.right,MAX_MACRO(clipbox.left,x));
	paintbox.right = MAX_MACRO(clipbox.left,MIN_MACRO(clipbox.right,x+cx));
	paintbox.bottom = MAX_MACRO(clipbox.top,MIN_MACRO(clipbox.bottom,y+cy));
		...
		if (cx==head.biWidth && cy==head.biHeight){ //NORMAL
			...
			if (pbase){
				...
				for(yy=0;yy<desth;yy++){
					...
					sy = MAX_MACRO(0L,(int32_t)floor(dy));
					...
					for(xx=0;xx<destw;xx++){
						dx = (xx+xmin-x)*fx;
						sx = MAX_MACRO(0L,(int32_t)floor(dx));
