...
void CxImage::RGBtoBGR(uint8_t *buffer, int32_t length)
{
	if (buffer && (head.biClrUsed==0)){
		uint8_t temp;
		length = MIN_MACRO(length,(int32_t)info.dwEffWidth);
		length = MIN_MACRO(length,(int32_t)(3*head.biWidth));
...
void CxImage::SetPalette(uint32_t n, uint8_t *r, uint8_t *g, uint8_t *b)
{
	...
	if (!b) b = g;
	RGBQUAD* ppal=GetPalette();
	uint32_t m=MIN_MACRO(n,head.biClrUsed);
...
void CxImage::SetPalette(rgb_color *rgb,uint32_t nColors)
{
	...
	RGBQUAD* ppal=GetPalette();
	uint32_t m=MIN_MACRO(nColors,head.biClrUsed);
	for (uint32_t i=0; i<m;i++){
...
void CxImage::SetPalette(RGBQUAD* pPal,uint32_t nColors)
{
	...
	memcpy(GetPalette(),pPal,MIN_MACRO(GetPaletteSize(),nColors*sizeof(RGBQUAD)));
	info.last_c_isvalid = false;
}
////////////////////////////////////////////////////////////////////////////////
/**
...
void CxImage::SetClrImportant(uint32_t ncolors)
{
	...
	case 1:
		head.biClrImportant = MIN_MACRO(ncolors,2);
		break;
	case 4:
		head.biClrImportant = MIN_MACRO(ncolors,16);
		break;
	case 8:
