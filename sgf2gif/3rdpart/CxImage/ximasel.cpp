...
bool CxImage::SelectionAddRect(RECT r, uint8_t level)
{
	...
	if (info.rSelectionBox.top <= r2.top) info.rSelectionBox.top = MAX_MACRO(0L,MIN_MACRO(head.biHeight,r2.top+1));
	if (info.rSelectionBox.left > r2.left) info.rSelectionBox.left = MAX_MACRO(0L,MIN_MACRO(head.biWidth,r2.left));
	if (info.rSelectionBox.right <= r2.right) info.rSelectionBox.right = MAX_MACRO(0L,MIN_MACRO(head.biWidth,r2.right+1));
	if (info.rSelectionBox.bottom > r2.bottom) info.rSelectionBox.bottom = MAX_MACRO(0L,MIN_MACRO(head.biHeight,r2.bottom));

	int32_t ymin = MAX_MACRO(0L,MIN_MACRO(head.biHeight,r2.bottom));
	int32_t ymax = MAX_MACRO(0L,MIN_MACRO(head.biHeight,r2.top+1));
	int32_t xmin = MAX_MACRO(0L,MIN_MACRO(head.biWidth,r2.left));
	int32_t xmax = MAX_MACRO(0L,MIN_MACRO(head.biWidth,r2.right+1));
...
bool CxImage::SelectionAddEllipse(RECT r, uint8_t level)
{
	...
	if (info.rSelectionBox.left > (xcenter - xradius)) info.rSelectionBox.left = MAX_MACRO(0L,MIN_MACRO(head.biWidth,(xcenter - xradius)));
	if (info.rSelectionBox.right <= (xcenter + xradius)) info.rSelectionBox.right = MAX_MACRO(0L,MIN_MACRO(head.biWidth,(xcenter + xradius + 1)));
	if (info.rSelectionBox.bottom > (ycenter - yradius)) info.rSelectionBox.bottom = MAX_MACRO(0L,MIN_MACRO(head.biHeight,(ycenter - yradius)));
	if (info.rSelectionBox.top <= (ycenter + yradius)) info.rSelectionBox.top = MAX_MACRO(0L,MIN_MACRO(head.biHeight,(ycenter + yradius + 1)));

	int32_t xmin = MAX_MACRO(0L,MIN_MACRO(head.biWidth,xcenter - xradius));
	int32_t xmax = MAX_MACRO(0L,MIN_MACRO(head.biWidth,xcenter + xradius + 1));
	int32_t ymin = MAX_MACRO(0L,MIN_MACRO(head.biHeight,ycenter - yradius));
	int32_t ymax = MAX_MACRO(0L,MIN_MACRO(head.biHeight,ycenter + yradius + 1));

	int32_t y,yo;
	for (y=ymin; y<MIN_MACRO(ycenter,ymax); y++){
...
bool CxImage::SelectionAddPolygon(POINT *points, int32_t npoints, uint8_t level)
{
	...
	while (i < npoints){
		...
		if (localbox.top < r2.top) localbox.top = MAX_MACRO(0L,MIN_MACRO(head.biHeight-1,r2.top+1));
		if (localbox.left > r2.left) localbox.left = MAX_MACRO(0L,MIN_MACRO(head.biWidth-1,r2.left-1));
		if (localbox.right < r2.right) localbox.right = MAX_MACRO(0L,MIN_MACRO(head.biWidth-1,r2.right+1));
		if (localbox.bottom > r2.bottom) localbox.bottom = MAX_MACRO(0L,MIN_MACRO(head.biHeight-1,r2.bottom-1));
	...
	if (info.rSelectionBox.top <= localbox.top) info.rSelectionBox.top = MIN_MACRO(head.biHeight,localbox.top + 1);
	if (info.rSelectionBox.left > localbox.left) info.rSelectionBox.left = MIN_MACRO(head.biWidth,localbox.left);
	if (info.rSelectionBox.right <= localbox.right) info.rSelectionBox.right = MIN_MACRO(head.biWidth,localbox.right + 1);
	if (info.rSelectionBox.bottom > localbox.bottom) info.rSelectionBox.bottom = MIN_MACRO(head.biHeight,localbox.bottom);
