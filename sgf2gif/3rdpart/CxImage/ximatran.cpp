		...
		for (xs = 0; xs < newWidth; xs+=RBLOCK) {       //for all image blocks of RBLOCK*RBLOCK pixels
			for (ys = 0; ys < newHeight; ys+=RBLOCK) {
				if (head.biBitCount==24) {
					...
					for (x = xs; x < MIN_MACRO(newWidth, xs+RBLOCK); x++){    //do rotation
						...
						for (y = ys; y < MIN_MACRO(newHeight, ys+RBLOCK); y++){
					...
					for (x = xs; x < MIN_MACRO(newWidth, xs+RBLOCK); x++){
...
bool CxImage::Rotate(float angle, CxImage* iDst)
{
	...
	leftTop.x = MIN_MACRO(MIN_MACRO(newP1.x,newP2.x),MIN_MACRO(newP3.x,newP4.x));
	leftTop.y = MIN_MACRO(MIN_MACRO(newP1.y,newP2.y),MIN_MACRO(newP3.y,newP4.y));
	rightBottom.x = MAX_MACRO(MAX_MACRO(newP1.x,newP2.x),MAX_MACRO(newP3.x,newP4.x));
	rightBottom.y = MAX_MACRO(MAX_MACRO(newP1.y,newP2.y),MAX_MACRO(newP3.y,newP4.y));
					   ...
					   bool const bKeepOriginalSize)
{
	...
	float minx = (float) MIN_MACRO(MIN_MACRO(newp[0].x,newp[1].x),MIN_MACRO(newp[2].x,newp[3].x));
	float miny = (float) MIN_MACRO(MIN_MACRO(newp[0].y,newp[1].y),MIN_MACRO(newp[2].y,newp[3].y));
	float maxx = (float) MAX_MACRO(MAX_MACRO(newp[0].x,newp[1].x),MAX_MACRO(newp[2].x,newp[3].x));
	float maxy = (float) MAX_MACRO(MAX_MACRO(newp[0].y,newp[1].y),MAX_MACRO(newp[2].y,newp[3].y));
	...
	default: // bilinear interpolation
		if (!(head.biWidth>newx && head.biHeight>newy && head.biBitCount==24)) {
			...
			for(int32_t y=0; y<newy; y++){
				...
				fY = y * yScale;
				ifY = (int32_t)fY;
				ifY1 = MIN_MACRO(ymax, ifY+1);
				dy = fY - ifY;
				for(int32_t x=0; x<newx; x++){
					fX = x * xScale;
					ifX = (int32_t)fX;
					ifX1 = MIN_MACRO(xmax, ifX+1);
					dx = fX - ifX;
	...
	for (int32_t y=0;y<head.biHeight;y++){
		...
		for (int32_t x=0;x<head.biWidth;x++){
			if (!errordiffusion){
				...
				c.rgbRed = (uint8_t)MIN_MACRO(255L,MAX_MACRO(0L,(int32_t)c.rgbRed + ((er*7)/16)));
				c.rgbGreen = (uint8_t)MIN_MACRO(255L,MAX_MACRO(0L,(int32_t)c.rgbGreen + ((eg*7)/16)));
				c.rgbBlue = (uint8_t)MIN_MACRO(255L,MAX_MACRO(0L,(int32_t)c.rgbBlue + ((eb*7)/16)));
				...
				for(int32_t i=-1; i<2; i++){
					...
					c.rgbRed = (uint8_t)MIN_MACRO(255L,MAX_MACRO(0L,(int32_t)c.rgbRed + ((er * coeff)/16)));
					c.rgbGreen = (uint8_t)MIN_MACRO(255L,MAX_MACRO(0L,(int32_t)c.rgbGreen + ((eg * coeff)/16)));
					c.rgbBlue = (uint8_t)MIN_MACRO(255L,MAX_MACRO(0L,(int32_t)c.rgbBlue + ((eb * coeff)/16)));
...
bool CxImage::Dither(int32_t method)
{
	...
	case 2:
	{
		...
		for (int32_t y = 0; y < head.biHeight; y++) {
			...
			for (int32_t x = 0; x < head.biWidth; x++) {
				...
				level = (uint8_t)MIN_MACRO(255, MAX_MACRO(0, (int32_t)nlevel));
	...
	case 7:
	{
		...
		int32_t scale = MAX_MACRO(0,(8-2*order));
		int32_t level;
		for (int32_t y=0;y<head.biHeight;y++){
			info.nProgress = (int32_t)(100*y/head.biHeight);
			if (info.nEscape) break;
...
bool CxImage::CropRotatedRectangle( int32_t topx, int32_t topy, int32_t width, int32_t height, float angle, CxImage* iDst)
{
	...
	startx = MIN_MACRO(topx, topx - (int32_t)(sin_angle*(double)height));
...
bool CxImage::Crop(int32_t left, int32_t top, int32_t right, int32_t bottom, CxImage* iDst)
{
	...
	int32_t startx = MAX_MACRO(0L,MIN_MACRO(left,head.biWidth));
	int32_t endx = MAX_MACRO(0L,MIN_MACRO(right,head.biWidth));
	int32_t starty = head.biHeight - MAX_MACRO(0L,MIN_MACRO(top,head.biHeight));
	int32_t endy = head.biHeight - MAX_MACRO(0L,MIN_MACRO(bottom,head.biHeight));
...
//					nx=MAX_MACRO(xmin,MIN_MACRO(nx,xmax));
//					ny=MAX_MACRO(ymin,MIN_MACRO(ny,ymax));
				}
				else { nx=-1;ny=-1;}
				if (head.biClrUsed==0){
